...
import { CLASSIC_GAMES } from '@/components/game/GameCarousel';
import { LiveWins } from '@/components/game/LiveWins';

interface ClassicGameCardProps {
  game: typeof CLASSIC_GAMES[0];
  index: number;
}

function ClassicGameCard({ game, index }: ClassicGameCardProps) {
  // Game gradient colors
  const getGameGradientColor = (gameId: string): string => {
    const colorMap: Record<string, string> = {
      "blackjack-ultimate": "#1A1A1A",
      "crown-and-anchor": "#8B4513",
      "european-roulette": "#1B5E20",
      "oasis-poker": "#00695C",
      "poker-texas-holdem": "#B71C1C",
      "war-of-cards": "#6A1B9A",
      "coin-flip": "#B8860B",
      "keno-classic": "#1B5E20",
      "mines-adventure": "#3E2723",
      "plinko-drop": "#1976D2",
    };
  ...
  return (
    <Link href={`/classics/${game.id}`} passHref>
      <motion.div
...
export default function ClassicsPage() {
  ...
  const filteredGames = useMemo(() => {
    if (!searchTerm.trim()) return CLASSIC_GAMES;
    
    return CLASSIC_GAMES.filter(game =>
  ...
  return (
    <>
      <NextSeo
        title="Classic Casino Games - SOLS.BET | Blockchain Table Games"
        description="Experience classic casino favorites on Solana blockchain. Play Blackjack, Roulette, Poker, Mines, Plinko and more with provably fair mechanics and instant payouts."
        canonical="https://sols.bet/classics"
        openGraph={{
          title: "Classic Casino Games - SOLS.BET",
          description: "Experience classic casino favorites on Solana blockchain with provably fair mechanics and instant payouts.",
          url: "https://sols.bet/classics",
          images: [{
            url: "https://sols.bet/seo-banner.png",
            width: 1200,
            height: 630,
            alt: "SOLS.BET Classic Casino Games"
          }],
          site_name: "SOLS.BET"
        }}
        twitter={{
        ...
        additionalMetaTags={[
          {
            name: "keywords",
            content: "classic casino games, blackjack, roulette, poker, mines, plinko, blockchain casino, crypto games"
          }
        ]}
      />
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 md:gap-6">
            {filteredGames.map((game, index) => (
              <ClassicGameCard key={game.id} game={game} index={index} />
            ))}
          </div>
          ...
          <div className="text-center mt-12 text-gray-400">
            Showing {filteredGames.length} of {CLASSIC_GAMES.length} classic games
          </div>
        </div>
      </div>
    </>
